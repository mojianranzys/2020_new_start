...
server <- function(input, output) {
  ...
  strong_part <- function(x){
    ...
    if(nrow(x)>0){
      ...
      if(length(part_c)>0){
        ...
        for(i in seq(nrow(partc_table))){
          ...
          poly_aa <- unlist(strsplit(dat_t[1],"----------"))[2]
          dat_a <- as.data.frame(strsplit(sub(poly_aa," ",dat_t)," "))[1,]
          dat_a <- t(dat_a)
          rownames(dat_a) <- NULL
          b_rep <- length(dat_t)
          dat_b <- rep(poly_aa,b_rep)
          colnames(dat_b) <- NULL
          dat_c <- as.data.frame(strsplit(sub(poly_aa," ",dat_t)," "))[2,]
          dat_c <- t(dat_c)
          rownames(dat_c) <- NULL
    ...
    all_result <- cbind(end_all_a,all_b,all_c)
